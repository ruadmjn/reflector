...
public class CheckReflection {
    ...
    private burp.Settings settings;
    IBurpExtenderCallbacks callbacks;

    public CheckReflection(burp.Settings settings, IExtensionHelpers helpers, IHttpRequestResponse iHttpRequestResponse, IBurpExtenderCallbacks callbacks) {
...
class Aggressive
{
    ...
    Aggressive(burp.Settings settings, IExtensionHelpers helpers, IHttpRequestResponse baseRequestResponse, IBurpExtenderCallbacks callbacks, List<Map> reflectedParameters) {
        this.helpers = helpers;
        this.callbacks = callbacks;
    ...
    private String checkRespone(String testRequest) {
        ...
        burp.ContextAnalyzer contextAnalyzer;
        int bodyOffset;
        try {
            String response = new burp.Client(callbacks).run(testRequest, host, this.port);
            Matcher matcher = this.pattern.matcher(response);
            bodyOffset = response.indexOf("\n\n") + 2;
            if ( settings.getCheckContext() && bodyOffset != 1) {
                contextAnalyzer = new burp.ContextAnalyzer( response.substring(bodyOffset).toLowerCase() );
            } else {
